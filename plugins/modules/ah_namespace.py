...
def main():
    ...
    if new_name:
        ...
        new_fields["name"] = module.get_item_name(existing_item)
    else:
    ...
    if groups:
        for group in groups:
            new_fields["groups"].append(
                {
                    "name": group,
                    "object_permissions": ["change_namespace", "upload_to_namespace"],  # Old style of group object
                    "object_roles": ["galaxy.collection_namespace_owner"],  # New style of group object
                }
            )
